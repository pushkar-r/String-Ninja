    ...
    <header className="sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur border-b border-slate-200 dark:border-slate-800">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center gap-3">
        ...
        <div className="hidden md:flex items-center gap-2 ml-auto shrink-0">
          ...
          <button onClick={toggle} className="px-3 py-2 rounded-xl text-sm bg-slate-900 text-white dark:bg-slate-200 dark:text-slate-900 whitespace-nowrap">
            {theme === 'dark' ? 'Light' : 'Dark'}
          </button>
        </div>
        <div className="md:hidden ml-auto flex items-center gap-2 shrink-0">
          <button onClick={()=>setMobileOpen(v=>!v)} className="px-3 py-2 rounded-xl text-sm border dark:border-slate-700">Menu</button>
          <button onClick={toggle} className="px-3 py-2 rounded-xl text-sm bg-slate-900 text-white dark:bg-slate-200 dark:text-slate-900 whitespace-nowrap">
            {theme === 'dark' ? 'Light' : 'Dark'}
          </button>
        </div>
